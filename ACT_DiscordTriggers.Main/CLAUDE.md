# CLAUDE.md — Main (WPF UI) assembly

The net48 `ACT_DiscordTriggers.Main` assembly. It is **byte-loaded from `libs/`** by the thin
bootstrap (`ACT_DiscordTriggers/`), not loaded by ACT directly — so ACT never `GetTypes()`-scans
it and it may freely derive from third-party deps. WPF view + theme + plugin host. The repo-root
`CLAUDE.md` covers the three-assembly architecture and build; this file covers the UI.
...
- `PluginImpl.cs` — the real plugin lifecycle (bridge discovery, diagnostics); mounts the view in
  ACT's WinForms `TabPage` via `ElementHost`. The bootstrap forwards `InitPlugin`/`DeInitPluginAsync`
  here by reflection. Not an `IActPluginV1` (that's the bootstrap).
- `AppInfo.cs` — version display surfaced to the Information tab via `{x:Static local:AppInfo.VersionDisplay}`.
- `ViewBehaviors.cs` — `static` attached properties (`PasswordBoxBinding`, `LogListCopy`, `LogAutoScroll`).
...
pack URIs name this assembly: `pack://application:,,,/ACT_DiscordTriggers.Main;component/...`.
`RootNamespace` is `ACT_DiscordTriggers` (so the view's `clr-namespace:ACT_DiscordTriggers` `local:`
refs resolve here), but `AssemblyName` is `ACT_DiscordTriggers.Main`.

## Load context (not an invariant here)

ACT only reflection-scans the bootstrap, never this assembly, so there is **no** GetTypes()
load-invariant to respect here — types may derive from `CommunityToolkit.Mvvm` etc. freely.
Conventions that still hold:
- Never subclass `Microsoft.Xaml.Behaviors`. It's XAML-only (`i:Interaction.Triggers` +
