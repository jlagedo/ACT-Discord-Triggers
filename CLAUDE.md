...
- `npm test` — JS suite (protocol, framing, op dispatch; plus a Windows-only lifecycle suite that spawns the real bridge). Independent of `dist/`. Lifecycle tests skip on non-Windows (Windows named pipes). The real-ONNX suites (`tts-synth.test.ts` synthesizes through `OnnxTts`; `tts-e2e.test.ts` spawns the bridge with the audio sink and drives `SetConfig`+`SpeakText` to a WAV) auto-skip unless the `sherpa-onnx-node` addon and the voice models are both present — models default to `E:\dev\sherpa-onnx-test\models`, override with `ACT_DT_MODELS_DIR`. `tts.test.ts` covers the parser/converter/sink/validation with neither.
