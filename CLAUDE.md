...
- **Config** — the single `SetConfig` op carries the whole `PluginSettings` POCO verbatim (token included). The bridge reads the fields it knows, ignores the rest, and owns all interpretation (fx dice roll, quality tier→bitrate, normalize target→LUFS). `Connect` takes no args and logs in from the config's token.
...
Sample format: the bridge's whole interior pipeline (decode → resample → fx → normalize → declick → mix → master limiter) works in **interleaved float32 stereo** — nominal [-1, 1] but allowed to exceed between stages so headroom is preserved (an fx tail or a hot mix sum is pulled back down, not clipped mid-chain). int16 lives only at the two edges, both via `audio-format.ts`: the `SpeakPcm` wire frame is widened on ingest (`int16ToFloat32`), and `PcmMixer` sums voices in float64 then performs the pipeline's single int16 quantization at its output (`floatToInt16`) right before the Opus encoder. The diagnostic WAV sink also converts float→int16 at its boundary. Don't reintroduce per-stage int16 round-trips; `floatToInt16` is the single quantization point and the natural home for future TPDF dither.

Master bus limiter: `limiter.ts` (`LookaheadLimiter`) is a channel-linked look-ahead brickwall limiter that `PcmMixer` runs on the summed float64 bus right before `floatToInt16`, so overlapping voices that sum past full scale are ridden down to a true-peak-safe ceiling instead of hard-clipping (the `floatToInt16` clamp stays as the backstop). It is bypassed on a bare `PcmMixer` (pure summing bus — keeps the mix-math unit tests delay-free) and armed by `discord-host` via `configureLimiter` from the live config on join + on change. Config: `limiterEnabled` + `limiterCeilingIndex` (0..3 → `LIMITER_CEILINGS_DB` `[-0.5,-1,-2,-3]` dBTP, default index 1 = −1 dBTP); additive config fields, no protocol bump. The limiter is independent of `normalize` (it catches inter-voice sum clipping even with normalize off). Exercise it end-to-end with `npm run limiter:e2e` (drives the real mixer with overlapping/edge signals, writes WAVs + asserts no clipping).

Auto-leveling loudness: `normalize.ts` measures per-clip loudness with ITU-R BS.1770 K-weighting (`k-weighting.ts` — a high-shelf + ~38 Hz high-pass biquad cascade per channel, then a channel-summed mean square with the −0.691 LUFS offset), so leveling is perceptually uniform across spectrally different clips. `measureLevel` returns that loudness as a linear full-scale-equivalent (its dB value is LUFS) plus a separate broadband sample `peak` (the unchanged clip-safety ceiling); `computeGain` lands the loudness on the user's target with no extra conversion. The `normalizeTarget` config is a positive magnitude negated to a LUFS target (default 17 → −17 LUFS). Per-voice neural-TTS loudness is baked in `ACT_DiscordTriggers.Core/Tts/onnx-voices.json` (`rmsDbfs` = LUFS, `peakDbfs` = broadband peak) so the streaming synth path levels with a fixed gain; re-measure it with `npm run tts:rms -- --bake` after any change to the loudness metric. The metric is bridge-internal — the C# side only sends `normalizeTarget` (no protocol change), but it owns the user-facing scale: the auto-level slider is labeled LUFS and the `PluginSettings` v2→v3 migration shifts existing saved targets to the new scale.
